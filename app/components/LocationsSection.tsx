                ...
                <div className="grid lg:grid-cols-2 gap-6 sm:gap-10 lg:gap-16 items-center">
                    ...
                    >
                        ...
                        <p className="text-gray-600 dark:text-gray-300 text-xs sm:text-sm lg:text-base mb-4 sm:mb-6 lg:mb-8 leading-relaxed max-w-xl">
                            Our rapidly expanding datacenter network spans Bangladesh, India, Singapore, Europe and the Americas,
                            delivering ultra-low latency and lightning-fast connections wherever you play.
                        </p>

                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                            <div>
                                <h3 className="icon-text-primary orbitron-font text-sm sm:text-base lg:text-lg mb-1.5 sm:mb-2 font-semibold">Bangladesh</h3>
                                <div className="space-y-0.5 sm:space-y-1">
                                    {locations
                                        .filter(loc => loc.region === "Bangladesh")
                                        .map((location, index) => (
                            ...
                            <div>
                                <h3 className="icon-text-primary orbitron-font text-sm sm:text-base lg:text-lg mb-1.5 sm:mb-2 font-semibold">India</h3>
                                <div className="space-y-0.5 sm:space-y-1">
                                    {locations
                                        .filter(loc => loc.region === "India")
                                        .map((location, index) => (
                            ...
                            <div>
                                <h3 className="icon-text-primary orbitron-font text-sm sm:text-base lg:text-lg mb-1.5 sm:mb-2 font-semibold">Singapore</h3>
                                <div className="space-y-0.5 sm:space-y-1">
                                    {locations
                                        .filter(loc => loc.region === "Singapore")
                                        .map((location, index) => (
                                            <LocationItem key={location.name} location={location} index={index + 6} />
                                        ))
                                    }
                                </div>
                            </div>
